...
const OUTPUT_DIR = path.join(process.cwd(), "models", "fsl_unified", "bilstm");

const SEQUENCE_LENGTH = 120;
const FEATURE_DIMENSION = 126;
const RANDOM_SEED = 2026;
const TEMPORAL_STEPS = Number.parseInt(process.env.UNIFIED_TEMPORAL_STEPS ?? "30", 10);
const HIDDEN_SIZE = Number.parseInt(process.env.UNIFIED_HIDDEN_SIZE ?? "32", 10);
const COMBINED_SIZE = HIDDEN_SIZE * 2;
const EPOCHS = Number.parseInt(process.env.UNIFIED_EPOCHS ?? "50", 10);
const LEARNING_RATE = Number.parseFloat(process.env.UNIFIED_LEARNING_RATE ?? "0.002");
const DROPOUT_RATE = Number.parseFloat(process.env.UNIFIED_DROPOUT ?? "0.2");
const EARLY_STOPPING_PATIENCE = Number.parseInt(process.env.UNIFIED_PATIENCE ?? "12", 10);
...
const saveOutputs = ({ labelsData, metadata, model, frameIndices, trainMetrics, valMetrics, testMetrics, history }) => {
  ...
  const config = {
    modelType: "unified-bilstm", description: "BiLSTM on unified dataset (28 alphabet + 105 FSL-105 signs).",
    architecture: {
      recurrentLayers: [
  ...
  writeJson(path.join(OUTPUT_DIR, "model.json"), {
    artifactType: "unified-bilstm", createdAt,
...
const main = () => {
  ...
  // Load FSL-105 data (labelIds need +28 offset)
  ...
  // Combine
  const allTrainRaw = [...alphaTrain.samples, ...fslTrainRaw];
  const allTestRaw = [...alphaTest.samples, ...fslTestRaw];

  // Split combined train into train/val
  const rng = mulberry32(RANDOM_SEED + 1);
  const order = shuffle(Array.from({ length: allTrainRaw.length }, (_, index) => index), rng);
  const valCount = Math.max(1, Math.floor(allTrainRaw.length * 0.15));
  const valOrder = new Set(order.slice(0, valCount));
  const trainAll = []; const valAll = [];
  for (let i = 0; i < allTrainRaw.length; i++) {
    if (valOrder.has(i)) valAll.push(allTrainRaw[i]);
    else trainAll.push(allTrainRaw[i]);
  }

  console.log("Training Unified BiLSTM (133 classes: 28 alphabet + 105 FSL-105)");
  ...
  console.log(`Dataset: ${metadata.splits.train.total} train + ${valCount} val + ${allTestRaw.length} test`);
  ...
  console.log("\nUnified BiLSTM training complete.");
