...
const OUTPUT_DIR = path.join(process.cwd(), "models", "fsl_unified", "bilstm_v2_hybrid");
...
const ORIGINAL_WEIGHT = Number.parseFloat(process.env.HYBRID_ORIGINAL_WEIGHT ?? "3.0");
const AUGMENTED_WEIGHT = Number.parseFloat(process.env.HYBRID_AUGMENTED_WEIGHT ?? "1.0");
...
const loadSplit = (samples, frameIndices) => {
  return samples.map((sample) => ({
    ...
    sampleWeight: sample.sampleWeight ?? 1,
...
const trainSampleFull = (model, sample, rng, classWeights, smoothing, lr, curriculumWeight, sampleWeight = 1) => {
  const { fwdResult, bwdResult, classifierInput, dropoutMask, probabilities } = forward(model, sample, { training: true, rng });

  const loss = computeLoss(probabilities, sample.labelId, model.outputClasses, smoothing) * sampleWeight;
  ...
  for (let classIndex = 0; classIndex < model.outputClasses; classIndex += 1) {
    deltaOutput[classIndex] = (probabilities[classIndex] - target[classIndex]) * classWeights[sample.labelId] * curriculumWeight * sampleWeight;
  }
...
const main = () => {
  ...
  const markSource = (samples, source) => samples.map((s) => ({ ...s, _source: source }));
  ...
  const fslTrainRaw = markSource(remapFsl(fslTrain.samples), "fsl");
  const fslTestRaw = markSource(remapFsl(fslTest.samples), "fsl");
  const alphaTrainRaw = markSource(alphaTrain.samples, "original");
  const alphaTestRaw = markSource(alphaTest.samples, "original");
  ...
  const allTestRaw = [...alphaTestRaw, ...fslTestRaw];

  const rng = mulberry32(RANDOM_SEED + 1);
  const order = shuffle(Array.from({ length: alphaTrainRaw.length + fslTrainRaw.length }, (_, index) => index), rng);
  const valCount = Math.max(1, Math.floor((alphaTrainRaw.length + fslTrainRaw.length) * 0.15));
  const valOrder = new Set(order.slice(0, valCount));
  const trainBase = []; const valAll = [];
  const baseSamples = [...alphaTrainRaw, ...fslTrainRaw];
  for (let i = 0; i < baseSamples.length; i++) {
    if (valOrder.has(i)) valAll.push(baseSamples[i]);
    else trainBase.push(baseSamples[i]);
  }

  const trainWeighted = trainBase.map((s) => {
    const w = s._source === "fsl" ? 1.0 : ORIGINAL_WEIGHT;
    return { ...s, sampleWeight: s.sampleWeight ?? w };
  });
  const augmentedWeighted = augmentedTrainRaw.map((s) => ({
    ...s, sampleWeight: s.sampleWeight ?? AUGMENTED_WEIGHT,
  }));
  const valWeighted = valAll.map((s) => ({ ...s, sampleWeight: 1 }));
  const trainAll = [...trainWeighted, ...augmentedWeighted];

  console.log("Training Unified BiLSTM v2 (Hybrid)");
  ...
  console.log(`Original weight: ${ORIGINAL_WEIGHT}, Augmented weight: ${AUGMENTED_WEIGHT}`);
  ...
  const valSamples = loadSplit(valWeighted, frameIndices);
  ...
  const effectiveTrainSize = trainSamples.reduce((s, x) => s + (x.sampleWeight ?? 1), 0);
  console.log(`Effective train size (weighted): ${effectiveTrainSize.toFixed(0)}`);
  ...
  for (let epoch = 1; epoch <= EPOCHS; epoch += 1) {
    ...
    for (const sampleIndex of trainOrder) {
      const sw = trainSamples[sampleIndex].sampleWeight ?? 1;
      const result = trainSampleFull(model, trainSamples[sampleIndex], trainRng, classWeights, LABEL_SMOOTHING, lr, curriculumWeight, sw);
