...
const OUTPUT_DIR = path.join(process.cwd(), "models", "fsl_unified_v2", "transformer_attention");
...
class TransformerAttentionModel {
  constructor(outputClasses) {
    ...
    this.posEnc = new Float32Array(TEMPORAL_STEPS * D_MODEL);
    for (let t = 0; t < TEMPORAL_STEPS; t++) for (let d = 0; d < D_MODEL; d++) {
      this.posEnc[t * D_MODEL + d] = d % 2 === 0 ? Math.sin(t / 10000 ** (d / D_MODEL)) : Math.cos(t / 10000 ** ((d - 1) / D_MODEL));
    }
    ...
    const attnW = this._init2d(D_MODEL, D_MODEL, 0.02, rng);
    const attnB = new Float32Array(D_MODEL);
    this.attention = { wq: this._init2d(D_MODEL, 1, 0.02, rng), wqB: 0 };
  ...
  forward(frames, returnAttn = false) {
    ...
    for (let t = 0; t < T; t++) {
      ...
      for (let d = 0; d < D_MODEL; d++) x[offset + d] += this.posEnc[t * D_MODEL + d];
    }
    ...
    for (const layer of this.layers) {
      ...
      for (let h = 0; h < NUM_HEADS; h++) {
        ...
        for (let ti = 0; ti < T; ti++) for (let tj = 0; tj < T; tj++) {
          ...
          attnOut[ti * D_MODEL + h * headDim] = s / Math.sqrt(headDim);
        }
      }
      ...
      for (let t = 0; t < T; t++) {
        ...
        for (let d = 0; d < D_MODEL; d++) { let v = layer.fb2[d]; for (let ff = 0; ff < FF_DIM; ff++) v += ff1Out[ff] * layer.ff2[ff * D_MODEL + d]; x[t * D_MODEL + d] += v; }
      }
    }

    const attnScores = new Float32Array(T);
    for (let t = 0; t < T; t++) {
      const v = x.subarray(t * D_MODEL, (t + 1) * D_MODEL);
      for (let d = 0; d < D_MODEL; d++) attnScores[t] += v[d] * this.attention.wq[d];
    }
    const attnWeights = softmax(attnScores);

    const pooled = new Float32Array(D_MODEL);
    for (let t = 0; t < T; t++) for (let d = 0; d < D_MODEL; d++) pooled[d] += attnWeights[t] * x[t * D_MODEL + d];
    ...
    const result = { probabilities: softmax(logits), attentionWeights: attnWeights };
    return returnAttn ? result : result.probabilities;
  }
}

const main = () => {
  console.log("Transformer + Attention — Training on Unified v2 (+ FSL v4.5)");
  ...
  console.log(`Arch: D=${D_MODEL}, heads=${NUM_HEADS}, layers=${NUM_LAYERS}, FF=${FF_DIM}`);

  const model = new TransformerAttentionModel(outputClasses);
  const history = [];
  ...
  for (let epoch = 1; epoch <= EPOCHS; epoch++) {
    ...
    for (const si of order) {
      const s = trainS[si];
      const probs = model.forward(s.frames, false);
  ...
  const config = {
    modelType: "transformer-attention",
    description: "Transformer + Additive Attention Pooling on Unified v2 with FSL v4.5",
    architecture: { dModel: D_MODEL, numHeads: NUM_HEADS, ffDim: FF_DIM, numLayers: NUM_LAYERS, temporalSteps: TEMPORAL_STEPS, outputClasses, pooling: "additive_attention" },
  ...
  console.log(`Transformer+Attention training complete. Output: ${OUTPUT_DIR}`);
};
