...
const OUTPUT_DIR = path.join(process.cwd(), "models", "roboflow_static");
const INPUT_SIZE = 126;
const HIDDEN1_SIZE = 64;
const HIDDEN2_SIZE = 32;
const EPOCHS = Number.parseInt(process.env.MLP_EPOCHS ?? "100", 10);
const BATCH_SIZE = Number.parseInt(process.env.MLP_BATCH_SIZE ?? "32", 10);
const LEARNING_RATE = Number.parseFloat(process.env.MLP_LEARNING_RATE ?? "0.01");
const MOMENTUM = Number.parseFloat(process.env.MLP_MOMENTUM ?? "0.9");
const RANDOM_SEED = Number.parseInt(process.env.MLP_SEED ?? "2027", 10);
...
const loadData = () => {
  if (!fs.existsSync(ROBOFLOW_DIR)) {
    console.error(`[mlp] Roboflow data directory not found at ${ROBOFLOW_DIR}`);
    console.error("[mlp] Run `node scripts/extract-roboflow-landmarks.mjs` first.");
    process.exit(1);
  }
  const labelsData = readJson(path.join(ROBOFLOW_DIR, "labels.json"));
  if (!labelsData) {
    console.error("[mlp] labels.json not found. Extract landmarks first.");
    process.exit(1);
  }
  const labelList = labelsData.labels || (labelsData.labels ? labelsData.labels : null);
  if (!labelList) {
    console.error("[mlp] labels.json missing labels array.");
    process.exit(1);
  }
  const labelToId = {};
  ...
  const loadSplit = (name) => {
    ...
    console.warn(`[mlp] Unknown format in ${name}.json, treating as empty.`);
    return { features: [], labels: [], count: 0 };
  };

  const train = loadSplit("train");
  const val = loadSplit("validation");
  const test = loadSplit("test");

  return { train, val, test, labels: labelList, labelToId, numClasses };
};
...
const createModel = (numClasses, rng) => {
  const w1 = xavierInit(INPUT_SIZE, HIDDEN1_SIZE, rng);
  const b1 = new Float32Array(HIDDEN1_SIZE);
  const w2 = xavierInit(HIDDEN1_SIZE, HIDDEN2_SIZE, rng);
  const b2 = new Float32Array(HIDDEN2_SIZE);
  const w3 = xavierInit(HIDDEN2_SIZE, numClasses, rng);
  const b3 = new Float32Array(numClasses);

  const opt = (size) => ({ v: new Float32Array(size) });

  return {
    w1, b1, w2, b2, w3, b3, numClasses,
    optW1: opt(w1.length), optB1: opt(b1.length),
    optW2: opt(w2.length), optB2: opt(b2.length),
    optW3: opt(w3.length), optB3: opt(b3.length),
  };
};
...
const forward = (model, input) => {
  const z1 = new Float32Array(HIDDEN1_SIZE);
  const a1 = new Float32Array(HIDDEN1_SIZE);
  ...
  const z2 = new Float32Array(HIDDEN2_SIZE);
  const a2 = new Float32Array(HIDDEN2_SIZE);
  for (let j = 0; j < HIDDEN2_SIZE; j++) {
    let s = model.b2[j];
    const off = j * HIDDEN1_SIZE;
    for (let i = 0; i < HIDDEN1_SIZE; i++) s += a1[i] * model.w2[off + i];
    z2[j] = s;
    a2[j] = relu(s);
  }
  ...
  for (let j = 0; j < model.numClasses; j++) {
    let s = model.b3[j];
    const off = j * HIDDEN2_SIZE;
    for (let i = 0; i < HIDDEN2_SIZE; i++) s += a2[i] * model.w3[off + i];
    logits[j] = s;
  }
...
const trainBatch = (model, batch, lr) => {
  ...
  const gradW3 = new Float32Array(model.w3.length);
  const gradB3 = new Float32Array(model.b3.length);
  let loss = 0;

  for (let s = 0; s < batchSize; s++) {
    const input = batch.features[s];
    const labelId = batch.labelIds[s];
    const { z1, a1, z2, a2, probs } = forward(model, input);

    loss += -Math.log(Math.max(probs[labelId], 1e-10));

    const delta3 = new Float32Array(model.numClasses);
    for (let j = 0; j < model.numClasses; j++) delta3[j] = probs[j] - (j === labelId ? 1 : 0);

    for (let j = 0; j < model.numClasses; j++) {
      gradB3[j] += delta3[j];
      const off = j * HIDDEN2_SIZE;
      for (let i = 0; i < HIDDEN2_SIZE; i++) gradW3[off + i] += delta3[j] * a2[i];
    }

    const delta2 = new Float32Array(HIDDEN2_SIZE);
    for (let i = 0; i < HIDDEN2_SIZE; i++) {
      let g = 0;
      for (let j = 0; j < model.numClasses; j++) g += model.w3[j * HIDDEN2_SIZE + i] * delta3[j];
      delta2[i] = g * reluDeriv(z2[i]);
    }
    for (let j = 0; j < HIDDEN2_SIZE; j++) {
      gradB2[j] += delta2[j];
      const off = j * HIDDEN1_SIZE;
      for (let i = 0; i < HIDDEN1_SIZE; i++) gradW2[off + i] += delta2[j] * a1[i];
    }

    const delta1 = new Float32Array(HIDDEN1_SIZE);
    for (let i = 0; i < HIDDEN1_SIZE; i++) {
      let g = 0;
      for (let j = 0; j < HIDDEN2_SIZE; j++) g += model.w2[j * HIDDEN1_SIZE + i] * delta2[j];
  ...
  applySgdMomentum(model.w3, gradW3, model.optW3.v);
  applySgdMomentum(model.b3, gradB3, model.optB3.v);
...
const writeBinaryWeights = (model, outputPath) => {
  const buffers = [
    ...
    Buffer.from(model.w3.buffer),
    Buffer.from(model.b3.buffer),
  ];
...
const writeTfjsModelJson = (numClasses, outputPath) => {
  const topology = {
    ...
    config: {
      name: "sequential_1",
      layers: [
        { class_name: "Dense", config: { name: "dense_1", units: HIDDEN1_SIZE, activation: "relu", input_shape: [INPUT_SIZE] } },
        { class_name: "Dense", config: { name: "dense_2", units: HIDDEN2_SIZE, activation: "relu" } },
        { class_name: "Dense", config: { name: "dense_3", units: numClasses, activation: "softmax" } },
      ]
    }
  };
  const modelJson = {
    ...
    weightsManifest: [{
      paths: ["weights.bin"],
      weights: [
        { name: "dense_1/kernel", shape: [INPUT_SIZE, HIDDEN1_SIZE] },
        { name: "dense_1/bias", shape: [HIDDEN1_SIZE] },
        { name: "dense_2/kernel", shape: [HIDDEN1_SIZE, HIDDEN2_SIZE] },
        { name: "dense_2/bias", shape: [HIDDEN2_SIZE] },
        { name: "dense_3/kernel", shape: [HIDDEN2_SIZE, numClasses] },
        { name: "dense_3/bias", shape: [numClasses] },
      ]
    }],
    format: "layers-model",
    generatedBy: "signlangvisual-roboflow-mlp-v1",
...
const main = () => {
  console.log("[mlp] Training Roboflow MLP (2 hidden layers)");
  console.log("[mlp]=".repeat(50));

  const data = loadData();
  const { train, val, test, labels, labelToId, numClasses } = data;

  if (train.count === 0) {
    console.error("[mlp] No training data found. Extract landmarks first.");
    process.exit(1);
  }

  console.log(`[mlp] Labels: ${numClasses}`);
  console.log(`[mlp] Train: ${train.count}, Val: ${val.count}, Test: ${test.count}`);
  console.log(`[mlp] Architecture: ${INPUT_SIZE}->${HIDDEN1_SIZE}->${HIDDEN2_SIZE}->${numClasses}`);
  console.log(`[mlp] Epochs: ${EPOCHS}, Batch: ${BATCH_SIZE}, LR: ${LEARNING_RATE}, Momentum: ${MOMENTUM}`);

  const { normalized: trainFeat } = normalizeFeatures(train.features);
  const { normalized: valFeat } = normalizeFeatures(val.features);
  const { normalized: testFeat } = normalizeFeatures(test.features);
  ...
  for (let epoch = 1; epoch <= EPOCHS; epoch++) {
    ...
    if (epoch % 10 === 0 || epoch === 1 || epoch === EPOCHS) {
      ...
      console.log(`[mlp] Epoch ${epoch}/${EPOCHS} - loss ${(epochLoss / batches).toFixed(4)} train_acc ${(trainEval.accuracy * 100).toFixed(2)}% val_acc ${(valEval.accuracy * 100).toFixed(2)}%`);
    }
  }
  ...
  console.log(`\n[mlp] Final Metrics:`);
  console.log(`[mlp]   Train accuracy: ${(trainEval.accuracy * 100).toFixed(2)}%`);
  console.log(`[mlp]   Validation accuracy: ${(valEval.accuracy * 100).toFixed(2)}%`);
  console.log(`[mlp]   Test accuracy: ${(testEval.accuracy * 100).toFixed(2)}%`);

  if (testEval.cm) {
    const perClass = computePerClassAccuracy(testEval.cm, labels);
    console.log(`\n[mlp] Per-class accuracy (top/bottom 5):`);
    const sorted = Object.entries(perClass).sort(([, a], [, b]) => b - a);
    console.log(`[mlp]   Top 5:`);
    for (let i = 0; i < Math.min(5, sorted.length); i++) console.log(`[mlp]     ${sorted[i][0]}: ${(sorted[i][1] * 100).toFixed(2)}%`);
    console.log(`[mlp]   Bottom 5:`);
    for (let i = Math.max(0, sorted.length - 5); i < sorted.length; i++) console.log(`[mlp]     ${sorted[i][0]}: ${(sorted[i][1] * 100).toFixed(2)}%`);

    const confused = getTopConfused(testEval.cm, labels);
    if (confused.length > 0) {
      console.log(`\n[mlp] Top 10 most confused pairs (actual → predicted):`);
      for (const c of confused) console.log(`[mlp]   ${c.actual} → ${c.predicted}: ${c.count} times`);
    }
  }
  ...
  writeJson(path.join(OUTPUT_DIR, "config.json"), {
    modelType: "roboflow-mlp",
    architecture: { inputSize: INPUT_SIZE, hidden1: HIDDEN1_SIZE, hidden2: HIDDEN2_SIZE, outputClasses: numClasses },
  ...
  writeJson(path.join(OUTPUT_DIR, "model_weights.json"), {
    ...
    w3: Array.from(model.w3, (v) => Number(v.toFixed(8))),
    b3: Array.from(model.b3, (v) => Number(v.toFixed(8))),
  });
  ...
  console.log(`\n[mlp] Model saved to ${OUTPUT_DIR}`);
  console.log(`[mlp]   weights.bin: ${binSize} bytes`);
  console.log(`[mlp]   model.json (TFJS), labels.json, config.json, metrics.json`);
  console.log("[mlp] Done.");
};
