...
const INPUT_DIR = path.join(process.cwd(), "datasets", "processed", "fsl_105");
const OUTPUT_DIR = path.join(process.cwd(), "models", "fsl_105", "bilstm");

const SEQUENCE_LENGTH = 120;
const FEATURE_DIMENSION = 126;
const RANDOM_SEED = 2026;
const TEMPORAL_STEPS = Number.parseInt(process.env.FSL_105_TEMPORAL_STEPS ?? "30", 10);
const HIDDEN_SIZE = Number.parseInt(process.env.FSL_105_HIDDEN_SIZE ?? "32", 10);
const COMBINED_SIZE = HIDDEN_SIZE * 2;
const EPOCHS = Number.parseInt(process.env.FSL_105_EPOCHS ?? "45", 10);
const LEARNING_RATE = Number.parseFloat(process.env.FSL_105_LEARNING_RATE ?? "0.002");
const DROPOUT_RATE = Number.parseFloat(process.env.FSL_105_DROPOUT ?? "0.2");
const EARLY_STOPPING_PATIENCE = Number.parseInt(process.env.FSL_105_PATIENCE ?? "10", 10);
const VALIDATION_SPLIT = Number.parseFloat(process.env.FSL_105_VALIDATION_SPLIT ?? "0.15");
...
const createLstmWeights = (rng) => {
  ...
  const wx = new Float32Array(FEATURE_DIMENSION * gateSize); const wh = new Float32Array(HIDDEN_SIZE * gateSize); const b = new Float32Array(gateSize);
  const wxScale = Math.sqrt(1 / FEATURE_DIMENSION); const whScale = Math.sqrt(1 / HIDDEN_SIZE);
...
const createModel = (outputClasses) => {
  ...
  const fwd = createLstmWeights(rng); const bwd = createLstmWeights(rng);
...
const saveOutputs = ({ labelsData, metadata, model, frameIndices, trainMetrics, valMetrics, testMetrics, history }) => {
  ensureDir(OUTPUT_DIR);
  const createdAt = new Date().toISOString();
  const config = {
    modelType: "fsl-105-bilstm", description: "BiLSTM v2 on FSL-105 dataset (105 signs).",
    architecture: {
      recurrentLayers: [
    ...
    randomSeed: RANDOM_SEED, validationSplit: VALIDATION_SPLIT,
  ...
  const metrics = {
    ...
    train: trainMetrics, validation: valMetrics, test: testMetrics, history, createdAt
  };

  const modelArtifact = {
    artifactType: "fsl-105-bilstm", createdAt,
...
const main = () => {
  ...
  if (!Array.isArray(labelsData.labels) || labelsData.labels.length < 2) throw new Error("Need at least 2 classes.");
  ...
  // Load all training data and split into train/validation
  const trainPayload = readJson(path.join(INPUT_DIR, "train.json"));
  const trainRaw = trainPayload.samples;

  const rng = mulberry32(RANDOM_SEED + 1);
  const order = shuffle(Array.from({ length: trainRaw.length }, (_, index) => index), rng);
  const valCount = Math.max(1, Math.floor(trainRaw.length * VALIDATION_SPLIT));
  const valOrder = new Set(order.slice(0, valCount));
  const trainAll = []; const valAll = [];
  for (let i = 0; i < trainRaw.length; i++) {
    if (valOrder.has(i)) valAll.push(trainRaw[i]);
    else trainAll.push(trainRaw[i]);
  }

  console.log("Training FSL-105 BiLSTM");
  ...
  console.log(`Split: train=${trainAll.length}, val=${valAll.length}, test=${metadata.splitCounts.test}`);

  const trainSamples = loadSplit(trainAll, frameIndices);
  const valSamples = loadSplit(valAll, frameIndices);
  const testPayload = readJson(path.join(INPUT_DIR, "test.json"));
  const testSamples = loadSplit(testPayload.samples, frameIndices);
  ...
  for (let epoch = 1; epoch <= EPOCHS; epoch += 1) {
    ...
    for (const sampleIndex of trainOrder) {
      const result = trainSampleFull(model, trainSamples[sampleIndex], trainRng);
      trainLoss += result.loss; trainCorrect += result.correct;
    }
    const valMetrics = evaluate(model, valSamples, labelsData.labels);
    const epochStats = { epoch, trainLoss: trainLoss / trainSamples.length, trainAccuracy: trainCorrect / trainSamples.length, valLoss: valMetrics.loss, valAccuracy: valMetrics.accuracy };
    history.push(epochStats);
    console.log(`Epoch ${epoch}/${EPOCHS} - loss ${epochStats.trainLoss.toFixed(4)} acc ${formatPercent(epochStats.trainAccuracy)} val_loss ${epochStats.valLoss.toFixed(4)} val_acc ${formatPercent(epochStats.valAccuracy)}`);

    if (valMetrics.loss < bestValLoss - MIN_VALIDATION_DELTA) { bestValLoss = valMetrics.loss; epochsWithoutImprovement = 0; }
  ...
  saveOutputs({ labelsData, metadata, model, frameIndices, trainMetrics, valMetrics, testMetrics, history });

  console.log("\nFSL-105 BiLSTM training complete.");
