...
const INPUT_DIR = path.join(process.cwd(), "models", "fsl_alphabet", "lstm");
const OUTPUT_DIR = path.join(process.cwd(), "models", "fsl_alphabet", "tfjs");
...
const main = async () => {
  console.log("Exporting LSTM model to TensorFlow.js format");
  ...
  const lstmModel = readJson(path.join(INPUT_DIR, "model.json"));
  const labelsData = readJson(path.join(INPUT_DIR, "labels.json"));
  const weights = lstmModel.weights;
  ...
  const wxTensor = tf.tensor2d(Float32Array.from(weights.wx), [FEATURE_DIMENSION, HIDDEN_SIZE * 4]);
  const whTensor = tf.tensor2d(Float32Array.from(weights.wh), [HIDDEN_SIZE, HIDDEN_SIZE * 4]);
  const bTensor = tf.tensor1d(Float32Array.from(weights.b));
  const wyTensor = tf.tensor2d(Float32Array.from(weights.wy), [HIDDEN_SIZE, OUTPUT_CLASSES]);
  const byTensor = tf.tensor1d(Float32Array.from(weights.by));

  model.layers[0].setWeights([wxTensor, whTensor, bTensor]);
  ...
  const modelJson = {
    ...
    generatedBy: "fsl-alphabet-export",
  ...
  fs.copyFileSync(
    path.join(INPUT_DIR, "labels.json"),
    path.join(OUTPUT_DIR, "labels.json")
  );
  console.log("Labels copied");

  model.dispose();

  const outputFiles = fs.readdirSync(OUTPUT_DIR);
  console.log("Output files:", outputFiles.join(", "));
  console.log("TFJS export complete.");
};
