...
### Phase 48 — Landing Page Interactivity Pass
Turned the landing page from a page you scroll into one you can operate. `InteractiveShowcaseSection.tsx` now runs the **real** translation engine in-page: visitors type any phrase and see actual `globalPipeline` gloss output with live language detection and timing (verified: "Mahal kita kaibigan" → LOVE / KITA / FRIEND, 9ms). The pipeline is pulled in via `import()` only on first interaction (focus, hover, or typing), so it never touches the landing page's initial bundle — deliberately consistent with the [[phase-log]] Phase 47 perceived-performance work. Falls back to the ambient demo loop when the input is empty. `RecognitionSequence.tsx` became a real control: clickable stage dots, click-to-advance, ←/→ keyboard stepping, pause-on-hover, explicit play/pause, and auto-advance that stops once the visitor takes over. `ResearchPipelineSection.tsx` stages are now selectable and drive an `aria-live` detail panel explaining what each of the 7 stages actually does. `AccessibilitySection.tsx` stopped asserting accessibility and started demonstrating it — a working contrast toggle and 3-step text-size control that visibly restyle a live sample card. New `useScrollSpy.ts` (IntersectionObserver) drives active-section highlighting in `LandingNav.tsx`, alongside a `useScroll`-backed reading-progress bar. New `SpotlightCard.tsx` gives feature cards pointer-tracked glow (mouse-only, decorative). **Every landing animation now honours `prefers-reduced-motion`** via framer-motion's `useReducedMotion` — count-ups jump to their final value, entrance transitions are skipped, and the ambient loops stop; fitting for a product built around accessibility. Also fixed a real bug: `#showcase` had no `id`, so the nav's "Translate" link and the hero's "Watch Demo" button both silently did nothing. Verified in-browser at 1440/1100/375px: no horizontal overflow, no console errors, scroll-spy/progress/scrubbing/live-translation/stage-selection/contrast+text-size all confirmed working. Lint clean on `src/components/landing`; `tsc --noEmit` clean repo-wide.
